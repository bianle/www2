<?php
    ...
    if ($dir_perm == 1) { //sorted
        ...
        if (bbs_is_member_read($brdarr) && no_member_read_perm($articles[1])) html_error_quit("本版驻版可读，非本版驻版用户不能查看本版文章！");
        $article = $articles[1];
    } else {
        $num = @intval($_GET["num"]);
        ...
        if (bbs_is_member_read($brdarr) && no_member_read_perm($articles[0])) html_error_quit("本版驻版可读，非本版驻版用户不能查看本版文章！");
        $article = $articles[0];
    }
